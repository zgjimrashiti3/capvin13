...
async function seed() {
    ...
    const itemRepo = AppDataSource.getRepository(menu_item_entity_1.MenuItem);
    ...
    const itemsByCat = [
        ...
        [pije, [
                ...
                { name: 'Santal Mollë 0.2l', imageUrl: IMG.freshJuice, price: 1.5, sortOrder: 5 },
                ...
                { name: 'Santal Boronicë 0.2l', imageUrl: IMG.lemonade, price: 1.5, sortOrder: 11 },
                { name: 'Santal Mollë 0.2l', imageUrl: IMG.freshJuice, price: 1.5, sortOrder: 12 },
    ...
    for (const [cat, items] of itemsByCat) {
        await itemRepo.save(items.map((i) => itemRepo.create({ categoryId: cat.id, isAvailable: true, ...i })));
        total += items.length;
    }
